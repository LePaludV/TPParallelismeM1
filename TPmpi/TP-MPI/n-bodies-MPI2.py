...
#Chaque processus on les même données, ils font les calcul pour une partie donnée
#  des data
# a la fin d'une itération on regroupe leurs data 
...
#Split et distribution des i à traité
if rank==0:
	step=split([i for i in range(0,nbbodies)],size)
else:
	step=None
local_step=comm.scatter(step,root=0)
...
for t in range(0, NBSTEPS):
	...
	tmpdata= []
	for i in local_step:
		forces.append((0,0)) # force[i]
		for j in range(i,nbbodies):
			#print(forces[j])
			[fix,fiy] = forces[i]; 
			[fjx,fjy] = forces[j]; 
			[dfx, dfy] = interaction(data[i], data[j])
			forces[i] = [fix+dfx, fiy+dfy]
			forces[j] = [fjx-dfx, fjy-dfy]
	...
	for i in local_step:	
		tmpdata.append(update(data[i], forces[i]))

	comm.barrier()
	data=unsplit(comm.allgather(tmpdata))
